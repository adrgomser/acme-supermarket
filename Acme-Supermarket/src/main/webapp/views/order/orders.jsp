...
<display:table pagesize="5" class="displaytag" keepStatus="true"
	...
	<security:authorize access="hasRole('CLERK')">
		<display:column> <jstl:if test="${row.clerk == null}">
			<a href="order/clerk/assign.do?orderId=${row.id}">
		...
		<display:column>
		<a href="order/clerk/deliver.do?orderId=${row.id}">
		<spring:message	code="clerk.deliver" />
			</a>
		</display:column>
		</security:authorize>
		<display:column>
		<a href="order/clerk/seeItems.do?orderId=${row.id}">
		<spring:message	code="clerk.items" />
			</a>
		</display:column>
