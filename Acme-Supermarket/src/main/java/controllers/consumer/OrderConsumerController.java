...
public class OrderConsumerController extends AbstractController {
	...
	public ModelAndView saveCreate(@Valid Order order, BindingResult binding) {
		ModelAndView result;
			...
			try {
				Date date=new Date();
				if(order.getSumPrice()<1&&order.getPaise()!=null){
					result = createEditModelAndView(order,"order.commit.error");
				}else{
					System.out.println("opasa por aqui");
				if(order.getCreditCard().getExpeditionYear()>=date.getYear()){
					if(order.getCreditCard().getExpeditionMonth()>=(date.getMonth()+1)||order.getCreditCard().getExpeditionYear()>date.getYear()){
						System.out.println("opasa por aqui1");
				...
				if(order.getPaise()!=null){
					order.getPaise().setUsed(true);
				}
				orderService.createOrderByShoppingCart(order.getCreditCard(),order.getName(), order.getAddress(),order.getPaise());
	...
	protected ModelAndView createEditModelAndView(Order o, String message) {
		...
		result.addObject("paises", consumerService.findByPrincipal().getPaises());
